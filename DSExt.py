...
# Provides additional functions for data structure manipulation

def stretchList(inlist, length, harmonic = 1):
	return list([inlist[int(index * (len(inlist)/(length/harmonic))) % len(inlist)] for index in range(length)])
...
def loopListUsingReverse(inlist):
	return inlist[:-1] + reverseList(inlist)[:-1]
